...
if(loginCheck($_COOKIE['username'], $_COOKIE['password'])) {
	if(isset($_POST['postmsg']) && isset($_POST['to'])) {
		...
		if(empty($_POST['postmsg']) || empty($_POST['to'])) { echo '<div class="popup-body-posted">The message is too short. <div style="font-size: 12px; margin-top:3px;">Tap to close.</div></div>'; }
		elseif(strlen($_POST['postmsg']) <= 1000) {
			...
			if(!empty($resultTo[1])) {
				...
				echo '<div class="popup-body">The username does not exist. <div style="font-size: 12px; margin-top:3px;">Tap to close.</div></div>';
			}
		} else {
			echo '<div class="popup-body">The message is too long. <div style="font-size: 12px; margin-top:3px;">Tap to close.</div></div>';
		}
	} else {
		echo '<div class="popup-body">You must enter a username and a password. <div style="font-size: 12px; margin-top:3px;">Tap to close.</div></div>';
	}
} else {
